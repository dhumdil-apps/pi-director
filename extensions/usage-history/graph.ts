 ...
 */
export function buildGraphModel(hourly: Map<number, Map<HourlyKey, HourlyCell>>, options: GraphOptions): GraphModel {
...
export function renderChart(model: GraphModel, options: ChartRenderOptions): string[] {
  ...
  const drawOrder = model.series
    ...
    .sort((a, b) => {
      const rank = (entry: { s: GraphSeries; i: number }) =>
        entry.s.key === TOTAL_SERIES_KEY
          ? Number.POSITIVE_INFINITY
          : entry.s.key === OTHER_SERIES_KEY
            ? -1
            : entry.s.total;
  ...
  const midLabel =
    plotWidth >= startLabel.length + endLabel.length + 14
