...
The dashboard starts with one concise quick reference: `❓ /help · ⚙️
/extension-settings · 📊 /usage`. `extensions.ts` still supplies the grouped
...
Automatic on interactive session start. Starts with shortcuts to `/help`,
`/extension-settings`, and `/usage`, then shows the working directory and recent
usage. `/help` opens a reference of the bundle's commands, shortcuts, and
extensions; `/context` is available on demand.
