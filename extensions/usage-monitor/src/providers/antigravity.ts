...
import { API_TIMEOUT_MS, createTimeoutController, formatReset } from "../utils.js";
...
export class AntigravityProvider extends BaseProvider {
  ...
  async fetchUsage(deps: Dependencies): Promise<FetchResult> {
    ...
    if (!data) {
      return this.result(lastStatus ? this.emptySnapshot(httpError(lastStatus)) : this.emptySnapshot(fetchFailed()));
    }
