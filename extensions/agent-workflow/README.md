...
The Agent preserves the initial goal, accepted follow-ups, and unresolved outcomes as cumulative scope. Follow-up work may add, defer, supersede, skip, fail, or complete a stable C outcome, but it may not silently erase or rename one. Stable Agent-chosen slugs (`Q-goal-scope`, `D-tighten-writes`, `C-diagnose-jobs`) make question, decision, and outcome lifecycles reviewable across turns. User and Agent see the same slug; option labels must not embed those ids.

Runtime does not parse checklist or decision status. It does parse the `**Current work:**` line for the Progress Tracker working row (in-flight phrase only; omitted idle/empty/wait) and `## Digest` for the mode-picker Show plan row (returns to the picker). HTML comments are omitted so the format marker cannot become Current work. The Agent interprets the free-form artifact as a whole, keeps Work log and both transcripts append-only, and leaves the artifact resumable without chat history after every turn.
