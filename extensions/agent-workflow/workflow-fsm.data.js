...
window.WORKFLOW_FSM_DATA = {
  ...
  "states": {
    ...
    "explore": {
      ...
      "userMode": "spec",
      "role": "primary",
      "summary": "Spec primary home: codebase research and fact-finding; may CALL decide; plan plus optional throwaway HTML under `.pi` — no project file mutations.",
      "permission": "read",
      "substates": [
        "symbolSearch",
        "evidenceGathering"
...
window.WORKFLOW_FLOW_GRAPH = {
  ...
  "subgraphs": {
    "align": {
      ...
      "title": "ALIGN",
      "version": "2.8.4",
      "summary": "Land after session entry CONTINUE (or RETURN_ALIGN / body loop). RUN evaluate (primary): D-review ask, User clarification, RECONCILE_SCOPE — never CALL next; never explore the codebase. WHEN research would be a long isolated dump, or the question is logic/state-feel or what UI should look like, MAY ask whether to spec a throwaway prototype here or offload research/prototype to a CLI child (agy or pi --print); user-stated intent is enough. WHEN ready to leave Align primary work: TO_GATE establish (secondary judge). establish: confirm/summarize only — NEVER CALL ask. establish judges: RETURN to evaluate (more Align work) OR CALL next (mode switch / handoff). Same-mode stay without next: ESC, Return to editor, or /mode.",
      "framing": false,
      "initial": "evaluate",
      "states": {
        "evaluate": {
        ...
        "proc-capture-turn": {
          ...
          "h": 56,
          "kind": "procedure",
          "summary": "IF message is workflow-generated or a command THEN RETURN.",
          "procedure": [
            "IF message is workflow-generated or a command THEN RETURN.",
            "Redact credentials, secret values, and attachment bodies.",
            "IF no named artifact exists OR the first envision scope ask has not completed THEN RETAIN captured message in session only; do not CREATE or EDIT .pi/plan/* until start.",
          ...
          "customData": {
            "sharedProcedure": "CAPTURE_TURN",
            "userMode": "align"
          }
        },
        "proc-reconcile-scope": {
          "id": "proc-reconcile-scope",
          ...
          "h": 56,
          "kind": "procedure",
          "summary": "COMPARE message with Goal, accepted follow-ups, and every unresolved C outcome.",
          "procedure": [
            "COMPARE message with Goal, accepted follow-ups, and every unresolved C outcome.",
            "IF unambiguous and additive THEN APPEND a stable C outcome without deleting or renaming earlier outcomes; RETURN resolved.",
            "IF mode = ALIGN THEN CALL ask from evaluate with concrete keep/defer/replace/resolve options; on cancel RETURN unresolved; else APPEND full exchange to User transcript; RECORD synthesis in Goal, Align, Decisions, Checklist; ANNOTATE superseded/deferred/skipped/failed C with reasons; NEVER erase; RETURN resolved.",
          ...
          "customData": {
            "sharedProcedure": "RECONCILE_SCOPE",
            "userMode": "align"
          }
        }
      },
      "transitions": [
        ...
        {
          "id": "align-evaluate->proc-ask",
          "from": "evaluate",
          "to": "proc-ask",
          "label": "CALL ask",
          "event": "CALL ask",
          "description": "Envision/evaluate prompts question picker",
          "customData": {
            "synthetic": true,
            "procedureEdge": true
          }
        ...
        {
          "id": "align-evaluate->proc-reconcile-scope",
          "from": "evaluate",
        ...
        {
          "id": "align-evaluate->proc-proceed-with-best",
          "from": "evaluate",
        ...
        {
          "id": "align-evaluate->proc-capture-turn",
          "from": "evaluate",
        ...
        {
          "id": "align-establish->proc-next",
          "from": "establish",
          "to": "proc-next",
          "label": "CALL next",
          "event": "CALL next",
          "description": "Establish secondary gate queues exit",
          "customData": {
            "synthetic": true,
            "procedureEdge": true
          }
    ...
    "spec": {
      ...
      "title": "SPEC",
      "version": "2.8.4",
      "summary": "Land on and RUN explore (primary): bounded research; decide when needed. If prototyping in-session: write throwaway HTML under `.pi` only (logic: one file, free-play plus guided tabs; UI: radical variants), never project source; link the path in Evidence. If the plan records Align yes for a CLI child: run the subagent skill (agy or pi --print); child writes `.pi/offload/` or `.pi/proto/`; link that path, do not dump stdout. WHEN ready to propose or leave Spec: TO_GATE elaborate. elaborate: draft Proposal/Checklist, RECORD_DECISION as needed, RUN CLOSE_OUT; link any `.pi` prototype path in Proposal. elaborate judges: RETURN explore OR CALL next (never CALL next from explore).",
      "framing": false,
      "initial": "explore",
      "states": {
        "explore": {
        ...
        "proc-record-decision": {
          ...
          "h": 56,
          "kind": "procedure",
          "summary": "IF choice is NOT (material AND reversible AND autonomous AND in scope) AND does not cross a consequential or safety boundary THEN RETURN.",
          "procedure": [
            "IF choice is NOT (material AND reversible AND autonomous AND in scope) AND does not cross a consequential or safety boundary THEN RETURN.",
            "CALL decide with the question, context, and 2-3 viable compared options.",
            "TREAT the returned pick as the working choice with review state unresolved.",
          ...
          "customData": {
            "sharedProcedure": "RECORD_DECISION",
            "userMode": "spec"
          }
        },
        "proc-close-out": {
          ...
          "h": 56,
          "kind": "procedure",
          "summary": "APPEND phase result, actual changed paths, checks run, checks not run, limitations, and concerns to Work log.",
          "procedure": [
            "APPEND phase result, actual changed paths, checks run, checks not run, limitations, and concerns to Work log.",
            "RECONCILE initial Goal, accepted follow-ups, every C outcome, and every D lifecycle from the artifact as a whole.",
            "FOR EACH C outcome: MARK completed only with evidence; OTHERWISE keep unresolved, or annotate deferred/skipped/failed with a reason.",
      ...
      "transitions": [
        ...
        {
          "id": "spec-explore->proc-decide",
          "from": "explore",
          "to": "proc-decide",
          "label": "CALL decide",
          "event": "CALL decide",
          "description": "Autonomous rationale logger",
          "customData": {
            "synthetic": true,
            "procedureEdge": true
          }
        ...
        {
          "id": "spec-explore->proc-record-decision",
          "from": "explore",
        ...
        {
          "id": "spec-explore->proc-capture-turn",
          "from": "explore",
          "to": "proc-capture-turn",
          "label": "CAPTURE_TURN",
          "event": "CAPTURE_TURN",
        ...
        {
          "id": "spec-elaborate->proc-close-out",
          "from": "elaborate",
          "to": "proc-close-out",
          "label": "CLOSE_OUT",
          "event": "CLOSE_OUT",
        ...
        {
          "id": "spec-elaborate->proc-next",
          "from": "elaborate",
          "to": "proc-next",
          "label": "CALL next",
          "event": "CALL next",
          "description": "Spec secondary gate queues next actions",
          "customData": {
            "synthetic": true,
            "procedureEdge": true
          }
    ...
    "vibe": {
      ...
      "title": "VIBE",
      "version": "2.8.4",
      "summary": "Land on and RUN execute (primary): implement accepted scope in project files; decide when needed. Do not treat project-tree throwaways as the Spec prototype path (those live under `.pi`). WHEN ready to verify or leave Vibe: TO_GATE examine. examine: run checks, RECORD evidence, RUN CLOSE_OUT. examine judges: RETURN execute OR CALL next (never CALL next from execute).",
      "framing": false,
      "initial": "execute",
      "states": {
        "execute": {
      ...
      "transitions": [
        ...
        {
          "id": "vibe-execute->proc-record-decision",
          "from": "execute",
          "to": "proc-record-decision",
          "label": "RECORD_DECISION",
          "event": "RECORD_DECISION",
        ...
        {
          "id": "vibe-execute->proc-capture-turn",
          "from": "execute",
          "to": "proc-capture-turn",
          "label": "CAPTURE_TURN",
          "event": "CAPTURE_TURN",
        ...
        {
          "id": "vibe-examine->proc-close-out",
          "from": "examine",
          "to": "proc-close-out",
          "label": "CLOSE_OUT",
          "event": "CLOSE_OUT",
        ...
        {
          "id": "vibe-examine->proc-next",
          "from": "examine",
          "to": "proc-next",
          "label": "CALL next",
          "event": "CALL next",
          "description": "Vibe secondary gate queues next actions",
          "customData": {
            "synthetic": true,
            "procedureEdge": true
          }
