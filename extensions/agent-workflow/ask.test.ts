...
import { type AskDetails, registerAsk } from "./ask.js";
...
describe("ask tool", () => {
	...
	it("shows the question with only the headlines, and maps the pick back to its option", async () => {
		const { run, selectSpy } = harness(async () => "Restore verification");
		const result = await run();
		expect(selectSpy).toHaveBeenCalledWith("Which one?", ["Keep the trim", "Restore verification"]);
