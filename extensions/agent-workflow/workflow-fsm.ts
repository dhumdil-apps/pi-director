 ...
 *
 * v2.8.4 — next requires ≥1 ranked action; empty next errors; omit still fill-in; other modes stay fallbacks.
 ...
 * v2.6.12 — drop primary self-loop transitions; body ⇄ only (TO_GATE/TO_HOME).
 * v2.6.6+ — primary homes + secondary gates; bi-body ALIGN/SPEC/VIBE; Align dual landing NEXT_ALIGN/RETURN_ALIGN.
 */

export const WORKFLOW_FSM_VERSION = "2.8.4";
...
/** entry = session once; primary/secondary = mode body. Tools start/ask/decide/next are procedures, not roles. */
...
export interface FsmState {
  ...
  kind: FsmStateKind;
  /**
   * Persisted User mode while in this guided step.
   * Required for primary/secondary. Entry keeps align as session seed.
   */
  userMode?: FsmUserMode;
  /** entry = session/handoff once; primary = mode home; secondary = judge gate (may CALL next). */
  role: FsmStateRole;
  summary: string;
  /**
...
export interface FsmSessionEntry {
  state: FsmStateId;
  label: string;
  summary: string;
  steps: string[];
...
export interface FsmModeBody {
  mode: FsmUserMode;
  label: string;
  /** Ordered guided state ids that compose this User mode body (no session entry). */
  states: FsmStateId[];
  /** Primary home landed after next→this mode. */
  primary?: FsmStateId;
  /** Secondary judge gate (return primary or CALL next). */
  secondary?: FsmStateId;
  /** Shared exit tool/procedure the secondary may CALL (not a guided state). */
  exitTool?: "next";
...
export interface WorkflowFsm {
  ...
  always: string[];
  turn: string[];
  /** Session/handoff entry (envision) — outside mode bodies. */
  sessionEntry: FsmSessionEntry;
...
export const WORKFLOW_FSM: WorkflowFsm = {
  ...
  summary:
    "Compound lifecycle: session entry ENVISION (outside mode bodies), then user modes ALIGN (evaluate ⇄ establish), SPEC (explore ⇄ elaborate), VIBE (execute ⇄ examine). " +
    "Secondary gates judge: return to primary or CALL next (mode switch). start/ask/decide/next are shared tools/procedures — not guided graph nodes. " +
    "New sessions and handoffs start at envision with persisted mode ALIGN. " +
  ...
  invariants: [
    ...
    "Runtime does not parse artifact prose (only time-spent envelope and ## Digest for Show plan).",
  ...
  sessionEntry: {
    ...
    label: "ENVISION",
    summary:
      "Session entry only (outside mode bodies): capture goal, one ask for scope, start/reuse artifact, then CONTINUE to Align primary or PWB to Spec/Vibe primary.",
    steps: [
  ...
  procedures: {
    ...
    CLOSE_OUT: [
      ...
      "NEVER CALL empty next. Rank at least one other mode or handoff; omitting next still opens fill-in follow-ups.",
  ...
  states: {
    envision: {
      ...
      role: "entry",
      summary:
        "Session entry only: capture goal, no extra file reads, CALL ask once about goal scope ahead (one CALL, batch independent questions), then start/reuse the named artifact before evaluate.",
      permission: "read",
      substates: ["goalCapture", "scopeAsk", "ensureArtifact"],
      procedure: [
    ...
    evaluate: {
      ...
      procedure: [
        ...
        "READ Digest, Goal, Decisions (unresolved D), Checklist, Work log residuals, and the kickoff prompt if any — from the plan artifact; `.pi/AGENTS.md` and `.pi/MEMORY.md` as needed.",
    ...
    establish: {
      ...
      procedure: [
        ...
        "ELSE CALL next with at least one ranked other mode or handoff — NEVER include align. NEVER CALL empty next.",
    ...
    elaborate: {
      ...
      procedure: [
        ...
        "ELSE CALL next with at least one other mode (never spec); prefer Align evaluate when unresolved D ids; include handoff when useful. NEVER CALL empty next.",
    ...
    examine: {
      ...
      procedure: [
        ...
        "ELSE CALL next with at least one other mode (never vibe); prefer Align evaluate when unresolved D ids; include handoff when useful. NEVER CALL empty next.",
      ],
    },
  },
  transitions: [
    {
      ...
      label: "PWB → EXPLORE",
      description:
        "Proceed-with-best from envision scope ask: ensure named artifact (runtime start if none), then route to SPEC primary (skips evaluate).",
      userMediated: true,
    },
    {
      ...
      label: "PWB → EXECUTE",
      description:
        "Proceed-with-best from envision scope ask: ensure named artifact (runtime start if none), then route to VIBE primary (skips evaluate).",
      userMediated: true,
    },
    {
      ...
      description:
        "The envision ask was answered and start/reuse has named the artifact; continue into Align primary home.",
    },
    {
    ...
    },
    {
      ...
      label: "ALIGN",
      description:
        "Align body ⇄: TO_GATE evaluate→establish; TO_HOME establish→evaluate. Establish judges return vs CALL next.",
      bidirectional: true,
    },
    {
    ...
    },
    {
      ...
      label: "SPEC",
      description:
        "Spec body ⇄: TO_GATE explore→elaborate; TO_HOME elaborate→explore. Elaborate judges return vs CALL next.",
      bidirectional: true,
    },
    {
  ...
  tools: [
    {
      name: "start",
      summary:
        "First .pi/plan write: named artifact after the first answered envision scope ask, or linked legacy continuation.",
      modes: ["any"],
    ...
    },
    {
      name: "ask",
      summary:
        "Native ALIGN question loop; used from envision (entry scope; may precede start) and evaluate (later asks).",
      modes: ["align"],
      gate: [
    ...
    },
    {
      ...
      modes: ["any"],
      gate: [
        "Empty next is an error; rank at least one other mode or handoff. Omitting next still opens fill-in follow-ups.",
  ...
  notes: [
    ...
    "Preferred agent path ends SPEC/VIBE via CLOSE_OUT on the secondary before CALL next with at least one ranked action. Omitting next still opens a fill-in picker; empty next is an error. Agent-driven Align ends via ask (envision/evaluate) or next (establish); Align establish idle is only runtime NEXT_ALIGN.",
...
export function formatWorkflowPrompt(fsm: WorkflowFsm = WORKFLOW_FSM): string {
  ...
  for (const state of Object.values(fsm.states)) {
    const modeBit = state.userMode ?? "no-userMode";
    lines.push(
      `### ${state.label} (${state.id}, ${modeBit}, ${state.role}, project ${state.permission})`,
      state.summary,
      "",
    );
