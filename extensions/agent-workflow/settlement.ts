 ...
 * Align silent-idle fallback without loading agent-api.
 */
 ...
 * explicit empty next (no picker) > Align fallback open_picker > none.
...
export function dispatchSettlement<TAnswer>(input: SettlementDispatchInput<TAnswer>): SettlementDispatch<TAnswer> {
  const { ask, nextQueued, nextSkip, mode } = input;
