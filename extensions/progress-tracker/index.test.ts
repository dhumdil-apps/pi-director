...
function attention(emitted: Array<[string, any]>): string | undefined {
  const [, payload] = emitted.findLast(
    ([name, value]) =>
      name === "powerbar:update" && value.id === "attention-span",
  )!;
...
describe("progress tracker indicator", () => {
  ...
  it("updates the single-line idle prompt once the User switches to Vibe", async () => {
  ...
  it("updates and reports the current workflow mode", async () => {
    const { handlers, listeners, emitted } = harness();
    const widgets: Array<[string, any]> = [];
    await handlers.get("session_start")![0]({}, ctxWith(widgets));
    listeners.get("agent-workflow:mode")![0]({ mode: "vibe" });
    expect(strip(indicator(widgets)[0])).toContain("[accent][VIBE]");
    const [, status] = emitted.findLast(
      ([name]) => name === "agent-status:update",
    )!;
    expect(status.mode).toBe("vibe");
  });

  it("derives Vibe from the mode entry seeded before a handoff session starts", async () => {
    const { handlers } = harness();
    const widgets: Array<[string, any]> = [];
    await handlers.get("session_start")![0](
      {},
      ctxWith(widgets, [modeEntry("vibe")]),
    );
    expect(strip(indicator(widgets)[0])).toContain("[accent]What’s up next?");
  });

  it("reconstructs the latest mode on tree changes, folding legacy phase values", async () => {
    const { handlers } = harness();
    const widgets: Array<[string, any]> = [];
    const branch = [modeEntry("execute")];
    const ctx = ctxWith(widgets, branch);
    await handlers.get("session_start")![0]({}, ctx);
    expect(strip(indicator(widgets)[0])).toContain("[accent][VIBE]");

    branch.push(modeEntry("plan"));
    await handlers.get("session_tree")![0]({}, ctx);
    expect(strip(indicator(widgets)[0])).toContain("[warning][SPEC]");
  });

  it("does not leak the previous mode into a replacement session", async () => {
    const { handlers, emitted } = harness();
    const widgets: Array<[string, any]> = [];
    await handlers.get("session_start")![0](
      {},
    ...
    expect(status).toMatchObject({ mode: undefined });
  });
  ...
  it("shows task work while active, cache age while idle, and resets with the session", async () => {
    ...
    try {
      ...
      await handlers.get("message_end")![0](
        { message: { role: "assistant", timestamp: 6_500 } },
        ctx,
      );
      now.mockReturnValue(7_000);
      await handlers.get("agent_settled")![0]({}, ctx);
      expect(strip(indicator(widgets, 160)[0])).toContain("[accent]ask 6s");
      ...
      await handlers.get("message_end")![0](
        { message: { role: "assistant", timestamp: 104_500 } },
        ctx,
      );
      now.mockReturnValue(105_000);
      await handlers.get("agent_settled")![0]({}, ctx);
      expect(strip(indicator(widgets, 160)[0])).toContain("[accent]ask 11s");
      expect(strip(indicator(widgets, 160)[0])).not.toContain("[accent] 0s");
  ...
  it("pauses task work and shows the static mode prompt plus cache age during user dialogs", async () => {
    const now = vi.spyOn(Date, "now").mockReturnValue(1_000);
    try {
      const { handlers, listeners, emitted } = harness();
      const widgets: Array<[string, any]> = [];
      ...
      await handlers.get("message_end")![0](
        { message: { role: "assistant", timestamp: 5_000 } },
        ctx,
      );
      listeners.get(CHECKPOINT_EVENT)![0]({
        action: "open",
        id: "approval-1",
        kind: "approval",
        timestamp: 5_000,
      ...
      const [, waitingStatus] = emitted.findLast(
        ([name]) => name === "agent-status:update",
      )!;
      ...
      listeners.get(USER_WAIT_EVENT)![0]({
        waiting: false,
        reason: "approval",
      });
      listeners.get(CHECKPOINT_EVENT)![0]({
      ...
      expect(strip(indicator(widgets, 200)[0])).toContain(
        "[dim] 2s[dim] · [accent]ask 1m 06s[dim] · [dim]spec 0s[dim] · [dim]vibe 0s",
      );
      ...
      await handlers.get("agent_start")![0]({}, ctx);
      expect(strip(indicator(widgets, 200)[0])).toContain(
        "[dim] 0s[dim] · [accent]ask 1m 07s[dim] · [dim]spec 0s[dim] · [dim]vibe 0s",
      );
    } finally {
      now.mockRestore();
    }
  ...
  it("persists task work and restores task time plus cache age in a replacement session", async () => {
    ...
    try {
      ...
      now.mockReturnValue(6_000);
      expect(strip(indicator(widgets, 160)[0])).toContain(
        "[dim] 5s[dim] · [accent]ask 5s",
      );
      await first.handlers.get("message_end")![0](
        { message: { role: "assistant", timestamp: 6_000 } },
        ctx,
      );
      ...
      expect(readPlanTiming(persisted)).toEqual({
        askMs: 5_000,
        specMs: 0,
        vibeMs: 0,
        unallocatedMs: 60_000,
      });
      ...
      expect(strip(indicator(restored, 160)[0])).toContain(
        "[dim] 0s[dim] · [accent]ask 5s",
      );
    } finally {
      now.mockRestore();
  ...
  it("splits an active run immediately when the User switches mode", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "pi-progress-modes-"));
    const path = join(cwd, ".pi", "plan", "debug-login.md");
    await mkdir(join(cwd, ".pi", "plan"), { recursive: true });
    ...
    try {
      ...
      now.mockReturnValue(7_000);
      listeners.get("agent-workflow:mode")![0]({ mode: "vibe" });
      expect(strip(indicator(widgets, 240)[0])).toContain(
        "[dim] 0s[dim] · [dim]ask 6s[dim] · [dim]spec 0s[dim] · [accent]vibe 0s",
      );

      now.mockReturnValue(10_000);
      expect(strip(indicator(widgets, 240)[0])).toContain(
        "[dim] 3s[dim] · [dim]ask 6s[dim] · [dim]spec 0s[dim] · [accent]vibe 3s",
      );
