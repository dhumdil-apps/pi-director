...
- **`/mode`** — Open the manual mode picker. Agent-recommended actions appear first when present, followed by every remaining mode, handoff, Return to editor (same as ESC), **Return → ❓ ALIGN** when not already in Align (establish, no agent start), and **Show plan** (## Digest only, then back to the picker). Selecting handoff prepares `/handoff <name>` in the editor; press Enter to execute it.
